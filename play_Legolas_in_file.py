...
from utils.model import DenoisingRMA, OdomEstimator_Legolas
...
if __name__ == "__main__":
    odom_path = "/home/luochangsheng/odom/Legged_odom/logs/2025-04-10-16-58-36_0.02s_no_pos_seq_actions/model_Legolas_2000.pt"
    ...
    odom_model_Legolas = torch.jit.load(odom_path).to(env.device)
    odom_model_Legolas.eval()
    ...
    for i in range(num_steps):       
        ...
        odom_pred_Legolas = odom_model_Legolas(odom_obs_history_Legolas, yaw_history[:, 2:])
        
        odom_pred_Legolas_pos = torch.stack(
            (
                torch.cos(abs_yaw_history[:, 1]) * odom_pred_Legolas[:, 0] - torch.sin(abs_yaw_history[:, 1]) * odom_pred_Legolas[:, 1] + pred_pos_history[:, -1, 0],
                torch.sin(abs_yaw_history[:, 1]) * odom_pred_Legolas[:, 0] + torch.cos(abs_yaw_history[:, 1]) * odom_pred_Legolas[:, 1] + pred_pos_history[:, -1, 1]
            ),
            dim=-1
        ) # x_i = d_i + x_i-1 [num_envs, 2]
        pred_pos_history = torch.roll(pred_pos_history, -1, dims=1)
        # print("done.shape", done.shape)
        pred_pos_history[:, -1, :] = odom_pred_Legolas_pos
