...
def ObsStackingEnvWrapperForOdom(base_env, obs_stacking, *args, **kwargs):
    class ObsStackingEnvImpl(base_env):
        def __init__(self, obs_stacking, *args, **kwargs):
            ...
            self.odom_obs_history = torch.zeros(self.num_envs, self.obs_stacking, 32, device=self.device)
        ...
        def reset(self):
            ...
            self.odom_obs_history[:, :, 6:32] = obs[:, 13:39].unsqueeze(1)
        ...
        def step(self, *args, **kwargs):
            ...
            self.odom_obs_history[done, :, 6:32] = obs[done, 13:39].unsqueeze(1)
            self.odom_obs_history[:, -1, 0:6] = obs[:, 0:6]
            self.odom_obs_history[:, -1, 6:32] = obs[:, 13:39]
