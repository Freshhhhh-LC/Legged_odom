...
class Controller:
    ...
    def run(self):
        ...
        self.dof_target[:], self.odom_pos = self.policy.inference(
            time_now=time_now,
    ...
    def _publish_cmd(self):
        while self.running:
            ...
            self.filtered_dof_target = self.filtered_dof_target * 0.9 + self.dof_target * 0.1

            for i in range(B1JointCnt):
                # q_range = self.cfg["common"]["torque_limit"][i] / self.cfg["common"]["stiffness"][i]
                # damping_offset = 
