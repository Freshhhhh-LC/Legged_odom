...
def plot_position(pos_history, step):
    pos = pos_history.cpu().numpy()
    rr.log_line_strip(f"position/step_{step}", pos, color=[0, 0, 255])

if __name__ == "__main__":
    rr.init("robocup_odom", spawn=True)
    ...
    latest_model_path = "/home/lcs/RCL_Project/robocup_odom/logs/2025-03-09-23-00-07/model_200.pth"
    ...
    for i in range(60000):
        for j in range(24):
            ...
            yaw_history = infos["yaw_history"].to(env.device)
            pos_history = infos["pos_history"].to(env.device)
            plot_position(pos_history[-1], i * 24 + j)  # 绘制坐标
