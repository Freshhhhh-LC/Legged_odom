...
if __name__ == "__main__":
    os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 确保使用第一个GPU设备
    ...
    odom_model = OdomEstimator(45 + 3, env.obs_stacking).to(env.device)
    ...
    buf.AddBuffer("odom_obs_history", (env.obs_stacking, 45), device=env.device)
    ...
    for i in range(60000):
        ...
        odom_loss_list = list()
        for j in range(20):
            odom_pred = odom_model(buf["odom_obs_history"], buf["yaw_history"], buf["start_mask"])
            odom_loss = F.mse_loss(odom_pred, buf["odom"])
            optimizer.zero_grad()
            odom_loss.backward()
            optimizer.step()
