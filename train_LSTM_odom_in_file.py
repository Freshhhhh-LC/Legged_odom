...
if __name__ == "__main__":
    ...
    data_dir = "/home/luochangsheng/odom/Enhanced_odom/data/train"
        ...
        for i in range(num_steps // 24):
            for j in range(24):
                ...
                with torch.no_grad():
                    ...
                    # LSTM模型输入 shape: [B, T, D]
                    # odom_obs_history_wys shape: torch.Size([32, 51, 46]), yaw_input shape: torch.Size([32, 50]), pos_input shape: torch.Size([32, 50, 2])
            ...
            for j in range(20):
                if use_pred_pos==False:
                    ...
                    # print("odom_obs_history_wys shape:", buf["odom_obs_history_wys"].shape)
                    # print("yaw_input shape:", yaw_input.shape)
            ...
            if i % 50 == 0:
                ...
                name = "model_wys_" + str(epoch) + NAME + str(i) + ".pt"
                scripted_model.save(os.path.join(dir, name))
                odom_model_wys.to(env.device)
                odom_model_wys.train()
                print(f"model saved at epoch {epoch}, iter {i}") 
